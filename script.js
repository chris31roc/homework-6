console.log("welcome to my weather application")

var cities = [];

function displayWeather() {
    var city = $("#city-input").val();
    const key = "d16ad09ee7b645660c202ca363ac444f";
    var queryURL = `https://api.openweathermap.org/data/2.5/weather?q=${city}&appid=${key}&units=imperial`;
    var forcastURL = `https://api.openweathermap.org/data/2.5/forecast?q=${city}&appid=${key}&units=imperial`
    
    //ajax call for forcast display in the dashboard
...
$.ajax({
    ...
    method: "GET"
      ...
      // UV Index URL
      var urlUV = `https://api.openweathermap.org/data/2.5/uvi?appid=${key}&lat=${response.coord.lat}&lon=${response.coord.lon}`;
      // UV Index ajax call
      $.ajax({
          url: urlUV,
          method: "GET"
        ...
        var uvColor = uvDisplay + `<span style="color: ${bkcolor}; padding: 0 7px 0 7px;">${response.value}</span>`;
        $('.UV').html(uvColor);    
      });
    });
  }
